...
import { Input } from '@/components/ui/aura/input';
...
import {
  ...
  SheetHeader,
  SheetTitle,
} from '@/components/ui/aura/sheet';
import { Skeleton } from '@/components/ui/aura/skeleton';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
    ...
    >
      ...
      >
        <div
          ...
          className={cn(
            'bg-sidebar rounded-lg p-2 gap-3 flex h-full w-full flex-col',
            'group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:border group-data-[variant=floating]:shadow-sm'
          )}
        >
          {children}
        </div>
...
function SidebarGroupLabel({ className, ...props }: ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'text-sidebar-foreground/70 ring-sidebar-ring flex h-8 shrink-0 items-center overflow-hidden px-2 text-xs font-medium outline-hidden transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
...
function SidebarMenuItem({
  className,
  'data-slot': dataSlot = 'sidebar-menu-item',
  'data-sidebar': dataSidebar = 'menu-item',
  ...props
...
// SidebarMenuSubItem uses the same implementation as SidebarMenuItem
...
function SidebarProjectSelector({
  ...
  return (
    <button
      data-slot="sidebar-project-selector"
      data-sidebar="project-selector"
      className={cn(
        'text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground flex h-9 w-full items-center gap-2 px-2 text-sm outline-hidden transition-[width,height,padding] focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
...
// Sidebar Search component (copy of ProjectSelector for now)
...
// Sidebar Pages wrapper component
function SidebarPages({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      data-slot="sidebar-pages"
      data-sidebar="pages"
      className={cn(
        'flex min-h-0 flex-1 flex-col gap-3 overflow-auto pb-1 group-data-[collapsible=icon]:gap-0',
        className
      )}
      {...props}
    />
...
// Sidebar Section component (base for MainPages, PinnedApps, GeneralPages)
...
// Sidebar Main Pages component
function SidebarMainPages(props: React.ComponentProps<'div'>) {
  return (
    <SidebarSection
      data-slot="sidebar-main-pages"
      data-sidebar="main-pages"
      {...props}
    />
  );
}

// Sidebar Pinned Apps (Big Swap Slot) component
function SidebarPinnedApps(props: React.ComponentProps<'div'>) {
  return (
    <SidebarSection
      data-slot="sidebar-pinned-apps"
      data-sidebar="pinned-apps"
      {...props}
    />
  );
}

// Sidebar General Pages component
function SidebarGeneralPages(props: React.ComponentProps<'div'>) {
  return (
    <SidebarSection
      data-slot="sidebar-general-pages"
      data-sidebar="general-pages"
      {...props}
    />
  );
}

// Sidebar Header component (contains Footer, Profile, Project selector, Search)
function SidebarHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      data-sidebar="header"
      className={cn(
        'flex flex-col gap-1 group-data-[collapsible=icon]:gap-1',
        className
      )}
      {...props}
    />
...
// Sidebar Footer component (used both inside Header and as standalone)
function SidebarFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      data-sidebar="footer"
      className={cn(
        'flex items-center gap-2 group-data-[collapsible=icon]:mt-3',
        className
      )}
      {...props}
    />
...
// Sidebar Footer component (used both inside Header and as standalone)
function SidebarHeaderInsert({
  className,
  ...props
}: React.ComponentProps<'div'>) {
  return (
    <div
      data-slot="sidebar-header-insert"
      data-sidebar="header"
      className={cn(
        'flex items-center gap-2 group-data-[collapsible=icon]:mb-3',
        className
      )}
      {...props}
    />
...
// Sidebar User Profile component (alias for Footer when used in Header context)
const SidebarUserProfile = SidebarFooter;
